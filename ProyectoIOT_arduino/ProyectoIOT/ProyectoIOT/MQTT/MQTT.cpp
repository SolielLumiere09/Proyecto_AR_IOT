...
const char* MQTT::ARDUINO_LUMINISCENSE = "ArduinoLuminiscense";
boolean MQTT::DOOR_STATUS = false;
...
float MQTT::readLuminiscense(){
	
	int lecture = analogRead(A1);
	float V = (float)(lecture * 5.f) / 1023.f;
	float L = (float)(1000.f * V - 0.5f)/15.1f;
...
void MQTT::sendLuminiscense(){
	Serial.print("luminescence = ");
	Serial.println(readLuminiscense());
	

	String payload = "luminescence = " + String(readLuminiscense());

	char attributes[payload.length() + 1];
	payload.toCharArray( attributes, payload.length() + 1);
	
	client->publish( ARDUINO_LUMINISCENSE, attributes );
